          ...
          {items.length === 0 ? (
            ...
            <div key={item.product.id} className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-bold text-gray-900 truncate">{item.product.model}</p>
                <p className="text-xs text-gray-400 mb-2">
                  PKR {formatPrice(item.plan === 'cash' ? item.product.price.cash_floor : item.product.price.retail)}
                </p>
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1 border border-gray-200 rounded-full px-2 py-1">
                    <button onClick={() => updateQty(item.product.id, item.qty - 1)}
                      aria-label="Decrease" className="text-gray-400 hover:text-gray-700 transition-colors">
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="text-sm font-medium w-4 text-center">{item.qty}</span>
                    <button onClick={() => updateQty(item.product.id, item.qty + 1)}
                      aria-label="Increase" className="text-gray-400 hover:text-gray-700 transition-colors">
                      <Plus className="h-3 w-3" />
                    </button>
                  </div>
                  <button onClick={() => removeItem(item.product.id)}
                    aria-label="Remove" className="text-gray-300 hover:text-red-500 transition-colors ml-auto">
