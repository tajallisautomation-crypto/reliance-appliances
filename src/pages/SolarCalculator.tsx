...
import { calcAllPlans, fmtPKR, roundTo100 as r100 } from '../lib/api'
...
export default function SolarCalculator() {
  ...
  const addCustom = () => {
    ...
    setItems(p=>[...p,{id:'c_'+Date.now(),name:custom.name,watts:parseInt(custom.watts)||0,
      qty:parseInt(custom.qty)||1,hours:parseFloat(custom.hours)||4,category:'Custom'}])
  ...
  const calc = async () => {
    ...
    try {
      ...
      const plans = calcAllPlans(total)
      setQuote({
        systemKW:kw, panels, inverterKW:invKW, batteryKWh:bKWh, type:sysType,
        costs:{panels:pCost,inverter:iCost,battery:bCost,installation:25000,total},
        savings:{unitsPerMonth:+(dailyU*30).toFixed(1),monthlySaving:msave,
          annualSaving:r100(msave*12),paybackMonths:Math.ceil(total/msave),
          paybackYears:+(Math.ceil(total/msave)/12).toFixed(1)},
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
      <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-yellow-500 text-white py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-amber-100 text-lg max-w-2xl mx-auto">Add your appliances and get an instant solar system quote.</p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 justify-center mb-8">
          {[{n:1,label:'Select Appliances'},{n:2,label:'Preferences'},{n:3,label:'Your Quote'}].map((s,i)=>(
            <div key={s.n} className="flex items-center gap-2">
              <button onClick={()=>step>s.n&&setStep(s.n as 1|2|3)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${step===s.n?'bg-orange-500 text-white shadow-lg scale-105':step>s.n?'bg-green-500 text-white':'bg-white text-gray-400 border'}`}>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-orange-100 p-4 space-y-3 max-h-96 overflow-y-auto">
                  ...
                  {items.map(item=>(
                    <div key={item.id} className="border border-gray-100 rounded-xl p-3">
                      ...
                      <div className="text-xs text-orange-600 mt-1 font-medium">{item.watts*item.qty}W x {item.hours}hrs = {(item.watts*item.qty*item.hours/1000).toFixed(2)} kWh/day</div>
                    </div>
                  ))}
                </div>
                {items.length>0&&(
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-orange-100 p-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {([{val:'on-grid',label:'On-Grid',desc:'No battery.'},{val:'hybrid',label:'Hybrid',desc:'Battery + grid.'},{val:'off-grid',label:'Off-Grid',desc:'Independent.'}] as const).map(s=>(
            ...
            <div className="flex gap-3">
              ...
              <button onClick={calc} disabled={loading} className="flex-1 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl py-3 font-semibold shadow-lg disabled:opacity-50">{loading?'Calculating...':'Generate Quote'}</button>
            </div>
          </div>
        )}
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-yellow-500 text-white rounded-3xl p-8 text-center">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[{v:quote.panels,l:'Panels (545W)'},{v:quote.inverterKW+'kW',l:'Inverter'},{v:quote.batteryKWh||'None',l:'Battery kWh'}].map((x,i)=>(
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-orange-100 p-6">
              ...
              <div className="space-y-3">
                {[{label:quote.panels+'x Solar Panels (545W)',val:quote.costs.panels},{label:quote.inverterKW+'kW Inverter',val:quote.costs.inverter},
                  ...(quote.costs.battery?[{label:quote.batteryKWh+'kWh Battery',val:quote.costs.battery}]:[]),
                  {label:'Installation & Wiring',val:quote.costs.installation}].map((row,i)=>(
                  <div key={i} className="flex justify-between items-center py-2 border-b border-gray-50"><span className="text-gray-600 text-sm">{row.label}</span><span className="font-semibold">{fmtPKR(row.val)}</span></div>
                ))}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-green-100 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[{l:'Units/Month',v:quote.savings.unitsPerMonth+' kWh'},{l:'Monthly Saving',v:fmtPKR(quote.savings.monthlySaving)},
                  {l:'Annual Saving',v:fmtPKR(quote.savings.annualSaving)},{l:'Payback',v:quote.savings.paybackYears+' yrs'}].map((s,i)=>(
                ...
                {Object.entries(quote.plans).map(([key,plan]: [string,any])=>(
                  <div key={key} className="border-2 border-blue-100 hover:border-blue-400 rounded-xl p-4 transition-all">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between border-t pt-2"><span className="text-gray-500">{plan.monthlyPayments}x Monthly</span><span className="font-bold text-lg text-blue-700">{fmtPKR(plan.monthly)}</span></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {!submitted?(
              ...
              <div className="bg-green-50 border border-green-200 rounded-2xl p-6 text-center">
                ...
                <button onClick={wa} className="mt-3 bg-green-500 text-white px-6 py-2 rounded-xl text-sm font-medium">WhatsApp Us</button>
              </div>
            )}
