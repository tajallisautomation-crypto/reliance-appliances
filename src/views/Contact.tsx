...
const CONTACT_INFO = [
  ...
  { icon: Mail,    label: 'Support Email',  value: 'support@tajallis.com.pk', href: 'mailto:support@tajallis.com.pk' },
  { icon: Mail,    label: 'Sales & B2B',   value: 'sales@tajallis.com.pk',   href: 'mailto:sales@tajallis.com.pk' },
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") console.error('[Contact]', err)
    } finally {
      setLoading(false)
    }
  }
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="contact-name" className="text-sm font-medium text-gray-700 block mb-1">Full Name *</label>
                  ...
                  <div>
                    <label htmlFor="contact-phone" className="text-sm font-medium text-gray-700 block mb-1">Phone Number *</label>
                    <input id="contact-phone" required value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))}
                      placeholder="+92 3XX XXXXXXX" type="tel" autoComplete="tel"
                      className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-400" />
                  </div>
                </div>
                <div>
                  <label htmlFor="contact-email" className="text-sm font-medium text-gray-700 block mb-1">Email (optional)</label>
                  <input id="contact-email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                    placeholder="your@email.com" type="email" autoComplete="email"
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-400" />
                </div>
                <div>
                  <label htmlFor="contact-subject" className="text-sm font-medium text-gray-700 block mb-1">Subject</label>
                ...
                <div>
                  <label htmlFor="contact-message" className="text-sm font-medium text-gray-700 block mb-1">Message *</label>
