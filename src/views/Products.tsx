...
export default function Products({ defaultSub }: { defaultSub?: string } = {}) {
  const sp                      = useSearchParams()
  const _params                 = useParams()
  const categorySlug            = _params?.categorySlug as string | undefined
  ...
  const brandSlug = _params?.brandSlug as string | undefined
  const category = categorySlug || sp?.get('category') || ''
  const brand    = brandSlug || sp?.get('brand') || ''
  ...
  const subSlug       = sp?.get('sub') || defaultSub || ''
  ...
  function goToCategory(catId: string) {
    ...
    if (!catId) {
      if (categorySlug) router.push('/products')
      else router.push(pathname)
      return
    }
    const cat = DEFAULT_CATEGORIES.find(c => c.id === catId || c.slug === catId)
    if (cat) { router.push(`/products/category/${cat.slug}`); return }
  ...
  function clearAll() {
    ...
    if (categorySlug) { router.push('/products'); return }
