...
import { useEffect, useState, startTransition, useRef, useCallback, type FC } from 'react';
...
const ProductDetail: FC<ProductDetailProps> = ({ initialProduct }) => {
  const params = useParams();
  const slug = params?.slug as string | undefined;
  const router = useRouter();

  // Ref holds the SSR product for one-time use on the matching slug.
  // Using a ref avoids adding `product` to the useEffect dependency array.
  const _ssrProductRef = useRef<Product | null>(initialProduct ?? null);
  ...
  useEffect(() => {
    ...
    // On first render, if SSR provided the product for this slug, skip the main fetch.
    const ssrP = _ssrProductRef.current;
    if (ssrP && ssrP.slug === slug) {
      _ssrProductRef.current = null; // consume once; client-nav to same slug re-fetches
      setLoading(false);
      getRelatedProducts(ssrP.id, ssrP.category, 4)
        .then(r => startTransition(() => { setRelated(r); setRelatedLoading(false); }))
        .catch(() => setRelatedLoading(false));
      getAlternativeProducts(ssrP.id, ssrP.category, ssrP.price.cash_floor, 4)
        .then(a => startTransition(() => { setAlternatives(a); setAltLoading(false); }))
        .catch(() => setAltLoading(false));
      getPriceHistory(ssrP.id)
        .then(ph => startTransition(() => { setPriceHistory(ph); setPriceHistoryLoading(false); }))
        .catch(() => setPriceHistoryLoading(false));
      Promise.resolve(supabase.from('reviews').select('rating').eq('product_id', ssrP.id))
        .then(({ data }) => {
    ...
    // Client-side navigation or no SSR data: full product fetch.
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 pb-28 md:pb-8">
      ...
      <nav className="flex items-center gap-1.5 text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6 overflow-x-auto no-scrollbar whitespace-nowrap">
        <Link href="/" className="hover:text-brand-600 shrink-0">Home</Link><span className="shrink-0">/</span>
        <Link href="/products" className="hover:text-brand-600 shrink-0">Products</Link><span className="shrink-0">/</span>
        <Link href={`/products/category/${catSlug}`} className="hover:text-brand-600 shrink-0">{catLabel}</Link><span className="shrink-0">/</span>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-gray-100">
                    {plan === 'cash' ? (
                      ...
                      <>
                        ...
                        <Link href="/installments#requirements"
                    ...
                    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 160 }}>
                      <defs>
                        <linearGradient id="ph-grad" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor="#0F2D52" stopOpacity="0.18" />
                          <stop offset="100%" stopColor="#0F2D52" stopOpacity="0" />
                        </linearGradient>
                      </defs>
                      {/* Filled area */}
                      <path d={areaPath} fill="url(#ph-grad)" />
                      {/* Line */}
                      <polyline points={polyline} fill="none" stroke="#0F2D52" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
                      {/* Dots */}
                      {pts.map((pt, i) => (
                        <circle key={i} cx={pt.x} cy={pt.y} r={i === pts.length - 1 ? 5 : 3.5}
                          fill={i === pts.length - 1 ? '#0F2D52' : '#fff'}
                          stroke="#0F2D52" strokeWidth="2" />
                      ))}
      ...
      {/* ── Why buy from Tajalli's ── */}
      <div className="mt-12 bg-brand-50 rounded-2xl px-5 sm:px-7 py-6">
        <h2 className="text-base font-black text-gray-900 mb-4">Why buy from Tajalli&apos;s?</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { icon: '✅', title: '100% Genuine',      desc: 'Official warranty, no grey imports' },
            { icon: '🚚', title: 'Same-day Delivery', desc: 'Fast delivery across all Karachi areas' },
            { icon: '🔧', title: 'Professional Setup', desc: 'Trained team installs and tests on-site' },
            { icon: '📞', title: 'After-Sale Support', desc: 'Dedicated support team — call or WhatsApp' },
          ].map(item => (
      ...
      {/* ── Product FAQ ── */}
      <div className="mt-10">
        <h2 className="text-base font-black text-gray-900 mb-4">Common Questions</h2>
        <div className="space-y-3">
          {[
            {
              q: `Is the ${p.brand} ${p.model || ''} available on installments?`,
              a: p.installments['12m'] || p.installments['6m']
                ? `Yes — installment plans are available starting from PKR ${Math.round((p.installments['12m'] ?? p.installments['6m'])!.monthly).toLocaleString('en-PK')}/month. Choose 2, 3, 6, or 12-month plans at checkout or ask on WhatsApp.`
                : 'Installment availability depends on the product price and your order. Ask on WhatsApp for the latest plan options.',
            },
            {
              q: 'Do you deliver and install in my area?',
              a: 'We deliver and install across all major Karachi areas including North Karachi, Federal B Area, Gulshan, Nazimabad, DHA, PECHS, Malir, Korangi, Orangi, and more. Message us on WhatsApp to confirm your area.',
            },
            {
              q: 'Is the product genuine with official warranty?',
              a: `Yes. All ${p.brand} products sold at Tajalli's are 100% genuine with full manufacturer warranty. We are an authorised dealer. ${p.warranty ? `Warranty for this model: ${p.warranty}.` : ''}`,
            },
            {
              q: 'How long does delivery take?',
              a: 'Same-day delivery is available for in-stock products in most Karachi areas. We confirm delivery time when you place an order on WhatsApp.',
            },
          ].map(({ q, a }) => (
            <details key={q} className="group bg-white border border-gray-100 rounded-xl overflow-hidden">
              ...
              <div className="px-4 pb-4 text-sm text-gray-600 leading-relaxed border-t border-gray-50 pt-3">{a}</div>
            </details>
          ))}
        </div>

        {/* Ask before buying CTA */}
        <div className="mt-4 flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500">Ask on WhatsApp before you buy — our team responds within minutes.</p>
          </div>
          <a
            href={`https://wa.me/923702578788?text=${encodeURIComponent(`Hi, I have a question about the ${p.brand} ${p.model || ''} before buying.`)}`}
            target="_blank"
            rel="noreferrer"
            className="shrink-0 inline-flex items-center gap-1.5 bg-[#25D366] hover:bg-[#1da84e] text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors"
          >
            Ask Now
          </a>
        </div>
      ...
      <div className="mt-8">
        <Link href="/products" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-brand-600 transition-colors">
