...
export default function AdminDashboard({
  ...
  async function load() {
    ...
    const [
      ...
      pendingRevRows, openTicketRows, pendingProofRows, pendingAppRows,
    ] = await Promise.all([
      ...
      safe(supabase.from('reviews').select('id').eq('status', 'pending')),
      safe(supabase.from('support_tickets').select('id').in('status', ['open', 'in_review', 'waiting_customer'])),
      safe(supabase.from('payment_proofs').select('id').eq('status', 'pending')),
      safe(supabase.from('installment_applications').select('id').eq('status', 'pending')),
    ]);
    ...
    setMetrics({
      ...
      pendingReviews: pendingRevRows.length,
      openTickets: openTicketRows.length,
      pendingProofs: pendingProofRows.length,
      pendingInstApps: pendingAppRows.length,
    });
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard label="Pending Reviews" value={metrics?.pendingReviews ?? 0} icon={Star}
              warn={(metrics?.pendingReviews ?? 0) > 0}
              bg="bg-orange-50" color="text-orange-500"
              sub={(metrics?.pendingReviews ?? 0) > 0 ? 'Awaiting moderation' : 'All moderated'}
              onClick={() => onNavigate('reviews')} />
            <StatCard label="Open Tickets" value={metrics?.openTickets ?? 0} icon={MessageCircle}
              warn={(metrics?.openTickets ?? 0) > 0}
              bg="bg-red-50" color="text-red-500"
              sub={(metrics?.openTickets ?? 0) > 0 ? 'Need response' : 'All resolved'}
              onClick={() => onNavigate('ops')} />
            <StatCard label="Payment Proofs" value={metrics?.pendingProofs ?? 0} icon={CreditCard}
              warn={(metrics?.pendingProofs ?? 0) > 0}
              bg="bg-blue-50" color="text-blue-600"
              sub={(metrics?.pendingProofs ?? 0) > 0 ? 'Awaiting verification' : 'All verified'}
              onClick={() => onNavigate('orders')} />
            <StatCard label="Instalment Apps" value={metrics?.pendingInstApps ?? 0} icon={ClipboardList}
              warn={(metrics?.pendingInstApps ?? 0) > 0}
              bg="bg-purple-50" color="text-purple-600"
              sub={(metrics?.pendingInstApps ?? 0) > 0 ? 'Awaiting review' : 'None pending'}
              onClick={() => onNavigate('installment_ledger')} />
          </div>
        )}
      </div>
