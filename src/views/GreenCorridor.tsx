...
const JOURNEY_STEPS = [
  ...
  },
  {
    number: '03',
    icon: Battery,
    iconColor: 'text-eco-300',
    bgColor: 'bg-eco-300/10',
...
export const GC_PACKAGES: GCPackage[] = [
  ...
  },
  {
    ...
    panelWatts: 620,
    panelCount: 20,
    inverterModel: 'Crown 12kW 3-Phase Hybrid Inverter',
    inverterWarranty: '2-year',
    batteryKwh: 10.24,
    batteryModel: '2 × Crown LiFePO4 5.12kWh Battery Bank (48V)',
    batteryWarranty: '5-year',
    acCount: 6,
    ...
    includes: [
      '20 × Crown Bi-Facial 620W Solar Panels (12.4kW peak)',
      'Crown 12kW 3-Phase Hybrid Inverter (2-yr warranty)',
      '2 × Crown LiFePO4 5.12kWh Battery — 48V, 10–16 hrs backup (5-yr warranty)',
      'Elevated 14-gauge galvanised steel mounting structure',
      'K-Electric Net Metering application & approved metering hardware',
      'All wiring, electrical equipment & transport',
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative bg-gray-950 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 py-24 md:py-32 text-center">
          <div className="inline-flex items-center gap-2 bg-eco-500/15 text-eco-300 px-4 py-2 rounded-full text-sm font-bold mb-8 uppercase tracking-widest">
            <Leaf className="w-4 h-4" /> Solar · Inverter ACs · Smarter Home
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-bold px-8 py-4 rounded-2xl transition-all">
              <CalendarCheck className="w-5 h-5" /> Book Free Consultation
            </button>
          </div>
          <p className="text-gray-600 text-xs mt-5">
            Prefer to chat? <a href={waConsult} target="_blank" rel="noreferrer" className="text-eco-300 hover:underline inline-flex items-center gap-1"><MessageCircle className="w-3 h-3" /> WhatsApp us</a>
          </p>
        </div>
      </div>
      ...
      <section className="bg-gray-950 py-20">
        <div className="max-w-5xl mx-auto px-4">
          <div className="text-center mb-14">
            <p className="text-eco-300 text-xs font-bold uppercase tracking-widest mb-3">The Green Corridor Journey</p>
                ...
                <div key={pkg.id}
                  ...
                  <div className={`p-6 flex flex-col flex-1 ${pkg.popular ? 'pt-8' : ''}`}>
                    ...
                    <button
                      ...
                      className="w-full py-3.5 rounded-2xl font-bold text-sm transition-all mb-2 flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-600 text-white">
      ...
      {/* ── BEFORE / AFTER BILL EXAMPLES ────────────────────────── */}
      <section className="max-w-5xl mx-auto px-4 py-16">
        <div className="text-center mb-10">
          <p className="text-eco-600 text-xs font-bold uppercase tracking-widest mb-3">Real-World Examples</p>
          <h2 className="text-3xl md:text-4xl font-black text-gray-900">Before & after — what your bill looks like.</h2>
          <p className="text-gray-500 mt-3">Based on actual Karachi households and businesses.</p>
        </div>
          ...
          {[
            {
              ...
              icon: '🏢',
              before: { bill: 12000, acs: 1, desc: '1 AC, fridge, lights & fans' },
              after:  { bill: 3600,  pkg: 'Starter (3kW)', reduction: '70%' },
              saving: 8400,
            },
            {
              profile: '3-Bedroom House',
              icon: '🏠',
              before: { bill: 28000, acs: 2, desc: '2 ACs, fridge, freezer, washing machine' },
              after:  { bill: 4200,  pkg: 'Home Complete (5kW)', reduction: '85%' },
              saving: 23800,
              highlight: true,
            },
            {
              profile: '5-Bedroom Villa / Office',
              icon: '🏛️',
              before: { bill: 65000, acs: 5, desc: '5 ACs, double fridge, business load' },
              after:  { bill: 3250,  pkg: 'Ultimate (10kW)', reduction: '95%' },
              saving: 61750,
            },
          ].map(ex => (
            <div key={ex.profile} className={`rounded-3xl p-6 border-2 ${ex.highlight ? 'border-eco-400 bg-eco-50' : 'border-gray-100 bg-white'} shadow-apple`}>
              <div className="text-3xl mb-3">{ex.icon}</div>
              <p className="font-bold text-gray-900 mb-1">{ex.profile}</p>
              <p className="text-xs text-gray-500 mb-4">{ex.before.desc}</p>

              <div className="flex items-center gap-2 mb-3">
                <div className="flex-1 bg-red-50 border border-red-100 rounded-xl p-3 text-center">
                  <p className="text-[10px] text-red-600 font-bold uppercase tracking-wide mb-1">Before</p>
                  <p className="text-xl font-black text-red-700">PKR {ex.before.bill.toLocaleString()}</p>
                  <p className="text-[10px] text-gray-500">per month</p>
                </div>
                <ArrowRight className="w-5 h-5 text-gray-400 shrink-0" />
                <div className="flex-1 bg-eco-100 border border-eco-200 rounded-xl p-3 text-center">
                  <p className="text-[10px] text-eco-600 font-bold uppercase tracking-wide mb-1">After</p>
                  <p className="text-xl font-black text-eco-700">PKR {ex.after.bill.toLocaleString()}</p>
                  <p className="text-[10px] text-gray-500">per month</p>
                </div>
              </div>

              <div className="bg-gray-900 text-white rounded-xl px-4 py-3 flex items-center justify-between">
                <span className="text-xs text-gray-300">Monthly saving</span>
                <span className="font-black text-eco-300 text-lg">PKR {ex.saving.toLocaleString()}</span>
              </div>
              <p className="text-[10px] text-gray-400 mt-2 text-center">{ex.after.pkg} · {ex.after.reduction} reduction</p>
            </div>
          ))}
        </div>
        <p className="text-xs text-gray-400 text-center mt-4">
          Examples assume Karachi tariff at PKR {UNIT_RATE}/kWh. Actual savings vary by usage pattern and site conditions.
        </p>
      </section>
      ...
      <section className="bg-gray-950 py-16 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-eco-300 text-xs font-bold uppercase tracking-widest mb-3">Phased Approach</p>
            <h2 className="text-3xl md:text-4xl font-black text-white">You don't have to do it all at once.</h2>
            <p className="text-gray-400 mt-3">Start with what makes financial sense today. Expand when you're ready.</p>
          </div>
          <div className="space-y-4">
            {[
              {
                phase: 'Phase 1',
                label: 'AC Upgrade (Quick Win)',
                cost: 'PKR 85,000 – 180,000',
                time: 'Day 1',
                saving: 'PKR 2,500 – 5,000/month',
                icon: '❄️',
                color: 'border-blue-500/30 bg-blue-500/5',
                labelColor: 'text-blue-300',
                desc: 'Replace your 1–2 non-inverter ACs with inverter models. Immediate 40–60% reduction in cooling costs. No solar, no complexity. Best ROI per rupee spent.',
              },
              {
                phase: 'Phase 2',
                label: 'Solar + Battery (Main Upgrade)',
                cost: 'PKR 560,000 – 1,150,000',
                time: 'Month 2–6',
                saving: 'PKR 10,000 – 20,000/month',
                icon: '☀️',
                color: 'border-eco-500/30 bg-eco-500/5',
                labelColor: 'text-eco-300',
                desc: 'Add a 3–5kW hybrid solar system with LiFePO4 battery. Your inverter ACs run on solar by day, battery by evening. Grid is backup only — bill drops 70–85%.',
              },
              {
                phase: 'Phase 3',
                label: 'Scale & Optimize (Optional)',
                cost: 'Expand existing system',
                time: 'Year 2+',
                saving: 'Near-zero bill',
                icon: '🌐',
                color: 'border-purple-500/30 bg-purple-500/5',
                labelColor: 'text-purple-300',
                desc: 'Expand to 7–10kW as your budget allows. Add more ACs, upgrade inverter refrigerators, or apply for KE Net Metering at 10kW+ to sell surplus back to the grid.',
              },
            ].map((step, i) => (
              <div key={step.phase} className={`rounded-2xl border p-5 ${step.color} flex gap-4 items-start`}>
                <div className="shrink-0 text-center">
                  <div className="text-2xl mb-1">{step.icon}</div>
                  <p className={`text-[10px] font-bold uppercase tracking-wide ${step.labelColor}`}>{step.phase}</p>
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <div className="bg-white/5 rounded-lg px-3 py-1.5">
                      ...
                      <p className="text-xs font-bold text-white">{step.cost}</p>
                    </div>
                    <div className="bg-white/5 rounded-lg px-3 py-1.5">
                      <p className="text-[10px] text-gray-500">Additional saving</p>
                      <p className="text-xs font-bold text-eco-300">{step.saving}</p>
                    </div>
                  </div>
                </div>
                {i < 2 && <div className="hidden md:flex w-px self-stretch bg-gray-700 mx-2" />}
              </div>
            ))}
          </div>
          <div className="mt-8 text-center">
            <button
              onClick={() => openBooking({ packageName: 'Green Corridor — Phased Consultation', notes: 'Upgrade timeline CTA' })}
              className="inline-flex items-center justify-center gap-2 bg-eco-500 hover:bg-eco-600 text-white font-bold px-8 py-4 rounded-2xl transition-all">
              <CalendarCheck className="w-5 h-5" /> Discuss Your Upgrade Path
            </button>
          </div>
        </div>
      </section>
      ...
      */}
      ...
      <section className="bg-gray-950 py-20 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <Leaf className="w-10 h-10 text-eco-300 mx-auto mb-6" />
