...
function _generateTags(brand: string, model: string, category: string, cc: string, specs: Record<string, string>, simplifiedName = ''): string {
  ...
  const cat = category.toLowerCase();
  void m;
  const tags: string[] = [b, brand, 'karachi', 'pakistan', 'tajallis', 'installment', 'easy installments', cat];
...
function _generateDescription(brand: string, model: string, simplifiedName: string, category: string, warranty: string, cc: string, specs: Record<string, string>): string {
  ...
  switch (cc) {
    ...
    default:
      return `The ${name} by ${brand} is a quality ${CANONICAL_DISPLAY[cc]?.toLowerCase() || category.toLowerCase()} designed for everyday Pakistani households. Built to local electrical standards (220V/50Hz). Warranty: ${warranty}. Available at Tajalli's, Karachi — on easy installments or cash. Call or WhatsApp for the latest price.`;
  }
}
...
export function enrichProduct(brand: string, model: string, category: string): Record<string, any> {
  ...
  const seo_title     = `${simplified_name} Price in Pakistan | Buy on Installments — Tajalli's Karachi`;
  const catDisplay    = cc !== 'unknown' ? (CANONICAL_DISPLAY[cc] ?? category) : category;
  const seo_desc      = `Buy the ${simplified_name} in Karachi at the best price. Available on easy installments at Tajalli's. ${warranty.split(',')[0]}. Call or WhatsApp now.`;
  const seo_keywords  = `${brand.toLowerCase()} ${catDisplay.toLowerCase()} karachi, ${simplified_name.toLowerCase()}, ${brand.toLowerCase()} ${model.toLowerCase()}, tajallis karachi, buy on installments karachi`;
      ...
      try {
        if (isUpdate) {
          ...
          if (csvItemName) {
            ...
            enriched.seo_title = `${csvItemName} Price in Pakistan | Buy on Installments — Tajalli's Karachi`;
            enriched.seo_desc  = `Buy the ${csvItemName} in Karachi at the best price. Available on easy installments at Tajalli's. ${enriched.warranty?.split(',')[0] ?? '1 year warranty'}. Call or WhatsApp now.`;
          }
