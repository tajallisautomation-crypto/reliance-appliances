...
// Single source of truth for all data fetching + calculation
// Aligns exactly with ApplianceStoreBrain.gs v5.0
// FIXES: CATEGORY_MAP added, calcAllPlans centralized, CATEGORY_MAP exported
...
export interface InstallmentPlan { total: number; advance: number; monthly: number; monthlyPayments: number; advancePct: number }
...
export interface Product { id: string; brand: string; model: string; name: string; category: string; subCategory: string; cashPrice: number; mrp: number; image: string; images: string[]; color: string; warranty: string; description: string; specs: string; tonnage: string; cubicFeet: string; inverter: boolean; inStock: boolean; featured: boolean; plans: Record<string, InstallmentPlan>; seoTitle: string; seoDesc: string; updatedAt: string }
export interface Category { id: string; name: string; icon: string; slug: string }

// Category short IDs → full names. MUST match CATEGORY_MAP in ApplianceStoreBrain.gs
...
function normaliseProduct(raw: any): Product {
  const basePrice = Number(raw.cashPrice || raw.cash_price || 0)
  return {
    ...
    subCategory: String(raw.subCategory || ''), cashPrice: basePrice,
    mrp: Number(raw.mrp || 0) || roundTo100(basePrice * 1.052),
    ...
    plans: (raw.plans && Object.keys(raw.plans).length > 0) ? raw.plans : calcAllPlans(basePrice),
...
async function apiFetch<T>(params: Record<string, string>, retries = 2): Promise<T> {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      return data as T
    } catch (e) { if (attempt === retries) throw e; await new Promise(r => setTimeout(r, 800 * (attempt + 1))) }
  }
